...
def get_all_graph_nodes(node_dicts):
    ...
    for modal in node_dicts:
        nodes_table_modals = pd.concat(
            [
                nodes_table_modals,
                graph_utils.nodes_table(modal, node_dicts[modal], modal_type_map),
            ]
        )
...
def get_all_graph_edges(
    ...
    node_links[dst_id] = node_links[dst_id].apply(
        lambda x: literal_eval(x)
    )  # convert dst_ids (loaded from csv in string format) to list format
    ...
    if scenes == True:
        ...
        scene_edges["dst_id"] = scene_edges["dst_id"].apply(
            lambda x: old_new_node_id_mapping[x]
        )
        scene_edges["src_id"] = scene_edges["src_id"].apply(
            lambda x: old_new_node_id_mapping[x]
        )
...
def get_new_edges(
    modal_embeds, new_old_node_id_mapping, hash_ids, sim_threshold, batch_size=500
):
    new_edges = pd.DataFrame(columns=["src_id", "dst_id"])

    for i in tqdm(
        range(0, len(modal_embeds), batch_size),
        desc="getting new similarity-based edges",
    ):
        edges_to_add = pd.DataFrame()
        end = min(i + batch_size, len(modal_embeds))

        embeds_batch = modal_embeds[i:end]
        cosine_sims_matrix = graph_utils.get_cosine_sim(
            embeds_batch, modal_embeds, np.array(range(i, end))
        )
    ...
    new_edges = (
        new_edges.explode("dst_id").dropna().drop_duplicates().reset_index(drop=True)
    )

    old_new_node_id_mapping = {v: k for k, v in new_old_node_id_mapping.items()}
    new_edges["dst_id"] = new_edges["dst_id"].apply(
        lambda x: old_new_node_id_mapping[x]
    )
    new_edges["src_id"] = new_edges["src_id"].apply(
        lambda x: old_new_node_id_mapping[x]
    )
...
def main_wrapper(
    ...
    def graph_builder(cfg):
        ...
        nodes_table, new_old_node_id_mapping, modal_node_ids = get_all_graph_nodes(
            node_dicts
        )
        with open(
            os.path.join(graph_location, "new_old_node_id_mapping.json"), "w"
        ) as outfile:
        ...
        edges_table = get_all_graph_edges(
            cfg, new_old_node_id_mapping, org=org, scenes=True
        )

        if new_edge_mode == "images" or new_edge_mode == "keywords":
            if sim_threshold == None:
                raise ValueError(
                    "If new_edge_mode is provided, sim_threshold must be a float between 0 and 1"
                )
            node_embeds, hash_ids = get_modal_embeds(new_edge_mode, node_dicts)
            new_edges = get_new_edges(
                node_embeds, hash_ids, sim_threshold, new_edges_batch_size
            )
            print(f"Added {len(new_edges)} new {new_edge_mode} links")

            edges_table = (
                pd.concat([edges_table, new_edges])
                .drop_duplicates()
                .reset_index(drop=True)
            )
            ...
            raise ValueError(
                'Invalid new_edge_mode input, expected "images" or "keywords" or None'
            )
